...
export const getSchedulesByEmail = async (req, res) => {
  try {
    let collection = db.collection('schedules')
    let schedules = await collection.find({ user: req.params.email }).toArray()

    if (!schedules.length) {
      return res.status(404).send({ message: 'No schedules found for this email' })
    }
...
export const createSchedule = async (req, res) => {
  try {
    ...
    res.status(201).send({ message: 'Schedule added', scheduleId: result.insertedId })
  } catch (err) {
    console.error(err)
...
export const updateScheduleByEmail = async (req, res) => {
  try {
    ...
    if (result.matchedCount === 0) {
      return res.status(404).send({ message: 'No schedule found for this email' })
    }
