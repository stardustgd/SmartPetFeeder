...
export const getManualFeedingsByEmail = async (req, res) => {
  try {
    ...
    if (!userFeeding) {
      return res.status(404).json({ message: 'No manual feeding record found for this user' })
    }

    res.status(200).json(userFeeding)
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch user manual feeding amount' })
  }
}
...
export const createManualFeeding = async (req, res) => {
    try {
        ...
        if (existingEntry) {
            return res.status(400).json({ error: 'Manual feeding entry already exists for this user' })
        }
        ...
        res.status(201).json({ message: 'Manual feeding created successfully', data: result })
    } catch (error) {
        console.error(error)
...
export const updateManualFeeding = async (req, res) => {
    try {
        ...
        if (!existingEntry) {
            return res.status(404).json({ error: 'No manual feeding entry found for this user' })
        }
        ...
        res.status(200).json({ message: 'Manual feeding updated successfully', data: result })
    } catch (error) {
        console.error(error)
